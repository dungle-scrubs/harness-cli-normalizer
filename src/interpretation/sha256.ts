/**
 * Pure SHA-256 (FIPS 180-4) over bytes, for verifying content-addressed
 * native blobs inside the interpretation layer's purity gate. Verified
 * against node:crypto in test/transcript/cursor.test.ts.
 */
...
const at = (words: Uint32Array, index: number): number => words[index] ?? 0;
...
export function sha256Hex(bytes: Uint8Array): string {
  ...
  for (let block = 0; block < padded.length; block += 64) {
    ...
    for (let i = 0; i < 64; i++) {
      const e = at(v, 4);
      const a = at(v, 0);
      const choose = (e & at(v, 5)) ^ (~e & at(v, 6));
      const t1 =
        (at(v, 7) + (rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)) + choose + at(K, i) + at(w, i)) >>> 0;
      const majority = (a & at(v, 1)) ^ (a & at(v, 2)) ^ (at(v, 1) & at(v, 2));
      const t2 = ((rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22)) + majority) >>> 0;
      v.copyWithin(1, 0, 7);
      v[4] = (at(v, 4) + t1) >>> 0;
      v[0] = (t1 + t2) >>> 0;
    }
    for (let i = 0; i < 8; i++) hash[i] = (at(hash, i) + at(v, i)) >>> 0;
  }
