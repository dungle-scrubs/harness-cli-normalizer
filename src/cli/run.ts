...
import { createHarnessSignalTarget } from "./signal-target.js";
...
export const run = async (harnessName: string, rawArgs: string[]): Promise<void> => {
  ...
  // wrapped to forward SIGINT/SIGTERM at the harness child explicitly (L6).
  // The approval observer spawns its helper after the harness child; that
  // later spawn must not retarget signals, so the first spawn wins.
  ...
  const onSig = async () => {
    ...
    abortController.abort();
    try {
      signalTarget.forward("SIGTERM");
      escalationTimer = setTimeout(() => {
        try {
          signalTarget.forward("SIGKILL");
        } catch {}
