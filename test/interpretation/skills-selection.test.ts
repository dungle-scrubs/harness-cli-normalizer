 ...
 * - codex/muse: refuse with the standard hint shape
 */
import { describe, expect, it } from "vitest";
import { buildLaunchArgv } from "../../src/interpretation/argv.js";
import { claudeSkillOverridesArg } from "../../src/interpretation/skills-selection.js";
...
describe("claude rendering", () => {
  ...
  it("refuses on the descriptor when skills is null", () => {
    for (const h of [codexCli, museCode]) {
      try {
        buildLaunchArgv(h, { prompt: "hi", skills: ["/root/a"] } as never);
        expect.unreachable(h.name);
      } catch (e) {
        const err = e as { issue: string; hint?: string };
        expect(err.issue).toBe("unsupported-option");
        expect(err.hint).toMatch(/no (call-time|per-skill) surface/);
      }
    }
  });
});
