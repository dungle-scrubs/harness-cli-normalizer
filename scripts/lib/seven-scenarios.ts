/**
 * The seven real-harness scenarios, as a library: smoke-seven runs them all
 * and prints a matrix; check-harnesses runs them per drifted harness and
 * reports pass/fail per descriptor claim.
 *
 * Each scenario names the descriptor claim it exercises, so a failure is
 * reported against the fact that is now wrong, not only the scenario name.
 *
 * Nondeterministic (live model turns), never part of the deterministic
 * suite. PLAN.md's "original seven" includes protocol-level scenarios
 * (interactive single-turn, path handoff, mid-flight exactly-once handoff)
 * that need the lucid-v2 substrate, which does not exist yet - those are
 * n/a at the runner level. The runner-testable seven:
...
export interface Scenario {
  name: string;
  /** The descriptor fact this scenario tests, as the reader sees it. */
  claim: string;
  run: (h: HarnessDescriptor) => Promise<Cell>;
}
...
export const modelFor = (h: HarnessDescriptor): string | undefined =>
  h.name === "pi"
...
export const collect = async (turn: AsyncIterable<HarnessEvent>): Promise<HarnessEvent[]> => {
...
// 7. error-propagation: an unspawnable binary yields an error event, a
//    transport failure, and done(127, failed) - the 0.2.0 contract
//    (runner-hardening.test.ts "a spawn that cannot start").
const errorProp = async (h: HarnessDescriptor): Promise<Cell> => {
  ...
  return {
    status: ok ? "pass" : "fail",
    detail: `done=${doneOf(events)} exit=${done?.kind === "done" ? done.exitCode : "?"} failure=${failure?.kind === "failure" ? failure.class : "none"}`,
  };
};

export const SCENARIOS: readonly Scenario[] = [
  { name: "single-turn", claim: "turn argv, identity, done", run: single },
  { name: "streaming", claim: 'capabilities.streamingByMode["headless-turn"]', run: streaming },
  { name: "tool-use", claim: "tool event decode", run: toolUse },
  ...
  { name: "session-cont(1proc)", claim: "sessionMode", run: sessionContinuity },
  { name: "resume-continuity", claim: "resume", run: resumeContinuity },
  { name: "kill-and-resume", claim: "resume after an abandoned turn", run: killResume },
  { name: "error-prop", claim: "runner error propagation", run: errorProp },
];

/** Run every scenario against one harness. A thrown scenario records a
 * fail with the error text, so one hung harness never stops the rest. */
export const runScenarios = async (h: HarnessDescriptor): Promise<Record<string, Cell>> => {
  const out: Record<string, Cell> = {};
  for (const s of SCENARIOS) {
    try {
      out[s.name] = await withTimeout(s.run(h));
    } catch (cause) {
      out[s.name] = { status: "fail", detail: String(cause).slice(0, 60) };
    }
  }
  return out;
};
